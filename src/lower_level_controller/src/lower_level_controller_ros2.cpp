/**
 * @file lower_level_controller_ros2.cpp
 * @author Abubakarsiddiq Navid shaikh
 * @date 2024-10-05
 * @brief ROS2 lower level controller matching ROS1 functionality
 */
...
class LowerLevelController : public rclcpp::Node
{
public:
  LowerLevelController()
  : Node("lower_level_controller_node")
  {
    // Declare parameters
    ...
    m_cmd_vel_sub = this->create_subscription<geometry_msgs::msg::Twist>(
      m_cmd_topic, 10,
      std::bind(&LowerLevelController::cmdVelCallback, this, std::placeholders::_1));
    
    m_e_stop_sub = this->create_subscription<std_msgs::msg::Bool>(
      m_emergency_stop_topic, 10,
      std::bind(&LowerLevelController::eStopCallback, this, std::placeholders::_1));
    ...
    m_odom_timer = this->create_wall_timer(
      ...
      std::bind(&LowerLevelController::publishOdometry, this));

    RCLCPP_INFO(this->get_logger(), "Lower Level Controller node has been started.");
  }
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<LowerLevelController>());
  rclcpp::shutdown();
  return 0;
}